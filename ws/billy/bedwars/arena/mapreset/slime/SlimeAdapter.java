...
package ws.billy.bedwars.arena.mapreset.slime;
    ...
    public void onEnable(final IArena arena) {
        ...
        Bukkit.getScheduler().runTaskAsynchronously(this.getOwner(), () -> {
            if (Bukkit.getWorld(arena.getWorldName()) != null) {
                ...
                slimePropertyMap.setInt(SlimeProperties.SPAWN_X, (int)Double.parseDouble(array[0]));
                slimePropertyMap.setInt(SlimeProperties.SPAWN_Y, (int)Double.parseDouble(array[1]));
                slimePropertyMap.setInt(SlimeProperties.SPAWN_Z, (int)Double.parseDouble(array[2]));
        ...
        Bukkit.getScheduler().runTaskAsynchronously(this.getOwner(), () -> {
            ...
            try {
                ...
                if (slimeLoader.worldExists(setupSession.getWorldName())) {
                    ...
                    Bukkit.getScheduler().runTask(this.getOwner(), () -> setupSession.getPlayer().sendMessage(ChatColor.GREEN + "Loading world from SlimeWorldManager container."));
                }
                else {
                    new File(Bukkit.getWorldContainer(), setupSession.getWorldName() + "/level.dat");
                    if (file.exists()) {
                        Bukkit.getScheduler().runTask(this.getOwner(), () -> setupSession.getPlayer().sendMessage(ChatColor.GREEN + "Importing world to the SlimeWorldManager container."));
