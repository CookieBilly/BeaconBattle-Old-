    ...
    public boolean execute(final String[] array, final CommandSender commandSender) {
        ...
        if (array.length == 0) {
            ...
            if (nearestTeam.isEmpty()) {
                ...
                player.spigot().sendMessage((BaseComponent)Misc.msgHoverClick(session.getPrefix() + "Or if you set the spawn and it wasn't found automatically try using: /bw " + this.getSubCommandName() + " <team>", "Set team upgrades NPC for a team.", "/" + this.getParent().getName() + " " + this.getSubCommandName() + " ", ClickEvent.Action.SUGGEST_COMMAND));
            ...
            if (session.getConfig().getYml().get("Team") != null) {
                ...
                for (final String s : Objects.requireNonNull(session.getConfig().getYml().getConfigurationSection("Team")).getKeys(false)) {
                    player.spigot().sendMessage((BaseComponent)Misc.msgHoverClick(ChatColor.GOLD + " " + '\u25aa' + " " + session.getTeamColor(s) + s + " " + ChatColor.getLastColors(session.getPrefix()) + "(click to set)", ChatColor.WHITE + "Upgrade npc set for " + TeamColor.getChatColor(Objects.requireNonNull(session.getConfig().getYml().getString("Team." + s + ".Color"))) + s, "/" + ws.billy.bedwars.BeaconBattle.mainCmd + " setUpgrade " + s, ClickEvent.Action.RUN_COMMAND));
                }
            }
        }
        else {
            ...
            if (session.getConfig().getYml().get("Team." + array[0] + ".Upgrade") != null) {
                ws.billy.bedwars.commands.Misc.removeArmorStand("upgrade", session.getConfig().getArenaLoc("Team." + array[0] + ".Upgrade"), null);
            }
            ws.billy.bedwars.commands.Misc.createArmorStand(string + " " + ChatColor.GOLD + "UPGRADE SET", player.getLocation(), null);
            session.getConfig().saveArenaLoc("Team." + array[0] + ".Upgrade", player.getLocation());
            player.sendMessage(session.getPrefix() + "Upgrade npc set for: " + string);
