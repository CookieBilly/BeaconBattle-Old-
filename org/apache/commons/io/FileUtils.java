...
public class FileUtils
{
    ...
    public static void touch(final File obj) {
        if (!obj.exists()) {
            IOUtils.closeQuietly(openOutputStream(obj));
        }
    ...
    public static boolean contentEqualsIgnoreEOL(final File file, final File file2, final String s) {
        ...
        try {
            if (s == null) {
                ...
                reader2 = new InputStreamReader(new FileInputStream(file2), Charset.defaultCharset());
            }
            else {
                reader = new InputStreamReader(new FileInputStream(file), s);
                reader2 = new InputStreamReader(new FileInputStream(file2), s);
            }
    ...
    private static void doCopyFile(final File file, final File obj, final boolean b) {
        ...
        try {
            ...
            IOUtils.closeQuietly(channel2, fileOutputStream, channel, fileInputStream);
        }
    ...
    public static void writeByteArrayToFile(final File file, final byte[] b, final int off, final int len, final boolean b2) {
        OutputStream openOutputStream = null;
        try {
            openOutputStream = openOutputStream(file, b2);
    ...
    public static void writeLines(final File file, final String s, final Collection<?> collection, final String s2, final boolean b) {
        ...
        try {
            ...
            ((FileOutputStream)openOutputStream).close();
        }
        finally {
            IOUtils.closeQuietly(openOutputStream);
        }
    ...
    public static boolean isSymlink(final File file) {
        ...
        return !file2.getCanonicalFile().equals(file2.getAbsoluteFile()) || isBrokenSymlink(file);
    }
    
    private static boolean isBrokenSymlink(final File file) {
        if (file.exists()) {
            return false;
        }
